...
//modifica:
//could use some improvements on the specific heat computation
...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double weight = exp(-2.0 * _beta * _J * ( _particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() + _H));
    if (_rnd.Rannyu() < 1 / (1 + weight)){                                              
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        ofstream coutt("../OUTPUT/TOTAL_ENERGY/total_energy.dat");
        ...
        ofstream coutpr("../OUTPUT/MAGNETIZATION/magnetization.dat");
        ...
        ofstream coutpr("../OUTPUT/SPECIFIC_HEAT/specific_heat.dat");
        ...
        ofstream coutpr("../OUTPUT/SUSCEPTIBILITY/susceptibility.dat");
  ...
  _global_av.zeros();
  _global_av2.zeros();
  _global_cv = .0;    //modifica
  _global_cv2 = .0;   //modifica
  _nattempts = 0;
  _naccepted = 0;
  return;
}
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    coutf.open("../INPUT/CONFIG/config.spin");    //modifica
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.ising");  //modifica
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
void System :: measure(){ // Measure properties
 ...
 if (_measure_cv){
    ...
    for (int i=0; i<_npart; i++){
      s_i = double(_particle(i).getspin());
      s_j = double(_particle(this->pbc(i+1)).getspin());
      cv += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
      }
      _measurement(_index_cv) = cv * cv;
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    for (int i = 0; i < _npart; i++){
...
void System :: averages(int blk){
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open("../OUTPUT/TOTAL_ENERGY/total_energy.dat",ios::app);
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    coutf.open("../OUTPUT/MAGNETIZATION/magnetization.dat",ios::app);
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
 if (_measure_cv){
    coutf.open("../OUTPUT/SPECIFIC_HEAT/specific_heat.dat",ios::app);
    average = _beta * _beta * (_average[_index_cv] - pow(_average[_index_tenergy]*(double)_npart, 2)) / (double)_npart; 
    _global_cv += average;
    _global_cv2 += average * average;
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    coutf.open("../OUTPUT/SUSCEPTIBILITY/susceptibility.dat",ios::app);
