...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream input("../INPUT/INPUT_EXAMPLES/input." + _phase); // Start reading
  transform(_phase.begin(), _phase.end(), _phase.begin(), ::toupper);

  ofstream couta("../OUTPUT/" + _phase + "/acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
...
void System :: initialize_velocities(){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open("../OUTPUT/CONFIG/velocities.out"); //modifica
    if(cinf.is_open()){
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        ofstream coutte("../OUTPUT/" + _phase + "/temperature.dat");
...
void System :: read_configuration(){
 ...
 if (_restart)
    cinf.open("../OUTPUT/CONFIG/config.xyz"); 
      ...
      cinf.open("../INPUT/CONFIG/config.xyz");  //modifica
...
void System :: ReadPhase(){
    ifstream phase("../INPUT/phase.dat");

  if(phase.is_open())
    phase >> _phase;
  else cerr << "PROBLEM: Unable to open INPUT file phase.dat"<< endl;
  phase.close();
  return;
}
...
void System :: measure(){ // Measure properties
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  //if (_measure_pressure)  _measurement(_index_pressure) = _rho * (2.0/3.0) * kenergy_temp + 16.*virial / ((double)k * _volume);
  if (_measure_pressure)  _measurement(_index_pressure) = _rho * (2.0/3.0) * kenergy_temp + 16.*virial / ((double)_volume);
...
void System :: averages(int blk){
  ...
  if (_measure_gofr){
    for (int i = _index_gofr; i < _index_gofr + _n_bins; i++){
      bin++;
      _block_av[i] /= (_rho * _npart * (4.*M_PI/3.) * (pow(_bin_size + bin*_bin_size, 3) - pow(bin*_bin_size, 3)));
    }
  }
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("../OUTPUT/" + _phase + "/temperature.dat",ios::app);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr and blk == _nblocks - 1){ 
    bin = 0; 
    coutf.open("../OUTPUT/" + _phase + "/gofr.dat", ios::app);
    for (int i = _index_gofr; i < _index_gofr + _n_bins; i++ ){
