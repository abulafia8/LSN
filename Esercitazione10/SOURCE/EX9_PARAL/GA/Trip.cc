...
//COSTRUTTORI
// Con un Random* (quello di pop) genera individui random
...
// Costruttore da usare in vector<Trip> new_pop
Trip::Trip(){
    for (int i = 0; i < N; i++)
        _trip.push_back(0);
    _map.push_back({.0});
}
// Honest to goodness constructor
Trip::Trip(vector<int> trip, vector<vec> map){
    _trip = trip;
    _map = map;
}
